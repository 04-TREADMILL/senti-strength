...
public class SentiStrength {
  ...
  public OutputVO computeSentimentScores(String sentence, String separator) {
    ...
    if (this.c.options.bgTrinaryMode) {
      output.setOutputMessage(
          output.getIPos() + separator + output.getINeg() + separator + output.getITrinary() +
              sRationale);
    } else {
      output.setOutputMessage(this.c.options.bgScaleMode ?
          output.getIPos() + separator + output.getINeg() + separator + output.getIScale() +
              sRationale :
  ...
  private void runMachineLearning(BaseCorpus c, String sInputFile, boolean bDoAll,
                                  int iMinImprovement,
  ...
  private void classifyAndSaveWithID(BaseCorpus c, String sInputFile, String sInputFolder,
                                     int iTextCol,
                                     int iIdCol) {
  ...
  private void parseOneText(BaseCorpus c, String sTextToParse, boolean bURLEncodedOutput) {
    ...
    if (bURLEncodedOutput) {
      ...
      System.out.println(new String(output.getOutputMessage().getBytes(StandardCharsets.UTF_8),
      ...
      try {
        while (true) {
          ...
          if (!c.options.bgForceUTF8) {
            ...
            System.out.println(
                new String(output.getOutputMessage().getBytes(StandardCharsets.UTF_8),
                    StandardCharsets.UTF_8));
          }
        }
  ...
  private void listenAtPort(BaseCorpus c, int iListenPort) {
    ...
    while (true) {
      ...
      try {
        while ((inputLine = in.readLine()) != null) {
          if (inputLine.indexOf("GET /") == 0) {
            ...
            try {
              decodedText =
                  URLDecoder.decode(inputLine.substring(5, lastSpacePos),
      ...
      if (c.options.bgForceUTF8) {
        out.print(new String(output.getOutputMessage().getBytes(StandardCharsets.UTF_8),
            StandardCharsets.UTF_8));
      } else {
