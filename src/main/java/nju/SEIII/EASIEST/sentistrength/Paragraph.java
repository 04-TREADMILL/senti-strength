...
public class Paragraph {
    ...
    public void addParagraphToIndexWithPosNegValues(
            UnusedTermsClassificationIndex unusedTermsClassificationIndex, int iCorrectPosClass,
            int iEstPosClass, int iCorrectNegClass, int iEstNegClass) {
        for (int i = 1; i <= this.igSentenceCount; ++i) {
            this.sentence[i].addSentenceToIndex(unusedTermsClassificationIndex);
        }

        unusedTermsClassificationIndex.addNewIndexToMainIndexWithPosNegValues(iCorrectPosClass,
    ...
    public void addParagraphToIndexWithScaleValues(
            UnusedTermsClassificationIndex unusedTermsClassificationIndex, int iCorrectScaleClass,
            int iEstScaleClass) {
        for (int i = 1; i <= this.igSentenceCount; ++i) {
            this.sentence[i].addSentenceToIndex(unusedTermsClassificationIndex);
        }

        unusedTermsClassificationIndex.addNewIndexToMainIndexWithScaleValues(iCorrectScaleClass,
                iEstScaleClass);
    }
    ...
    public void addParagraphToIndexWithBinaryValues(
            UnusedTermsClassificationIndex unusedTermsClassificationIndex, int iCorrectBinaryClass,
            int iEstBinaryClass) {
        for (int i = 1; i <= this.igSentenceCount; ++i) {
            this.sentence[i].addSentenceToIndex(unusedTermsClassificationIndex);
        }

        unusedTermsClassificationIndex.addNewIndexToMainIndexWithBinaryValues(iCorrectBinaryClass,
                iEstBinaryClass);
    }
    ...
    public int addToStringIndex(StringIndex stringIndex, TextParsingOptions textParsingOptions,
        ...
        for (int i = 1; i <= this.igSentenceCount; ++i) {
            iTermsChecked +=
                    this.sentence[i].addToStringIndex(stringIndex, textParsingOptions, bRecordCount,
                            bArffIndex);
        }
    ...
    public void addParagraphToIndexWithTrinaryValues(
            UnusedTermsClassificationIndex unusedTermsClassificationIndex, int iCorrectTrinaryClass,
            int iEstTrinaryClass) {
        for (int i = 1; i <= this.igSentenceCount; ++i) {
            this.sentence[i].addSentenceToIndex(unusedTermsClassificationIndex);
        }

        unusedTermsClassificationIndex.addNewIndexToMainIndexWithTrinaryValues(iCorrectTrinaryClass,
                iEstTrinaryClass);
    }
     ...
     */
    public void setParagraph(String sParagraph, ClassificationResources classResources,
        ...
        for (iPos = 0; iPos < sParagraph.length(); ++iPos) {
            ...
            if (!sNextSentence.equals("")) {
                ...
                this.sentence[this.igSentenceCount].setSentence(sNextSentence, this.resources,
                        this.options);
                sNextSentence = "";
        ...
        if (this.igSentenceCount != 0) {
            ...
            for (iNegTot = 1; iNegTot <= this.igSentenceCount; ++iNegTot) {
                ...
                if (this.options.bgExplainClassification) {
                    this.sgClassificationRationale.append(this.sentence[iNegTot].getClassificationRationale() + " ");
                }
            }
            ...
            if (var10000 == 1) {
                ...
                if (this.options.bgExplainClassification) {
                    this.sgClassificationRationale.append("[result = average (" + iPosTotal + " and " + iNegTotal + ") of " + iSentencesUsed + " sentences]");
                }
            } else {
                var10000 = this.options.igEmotionParagraphCombineMethod;
                this.options.getClass();
                if (var10000 == 2) {
                    ...
                    if (this.options.bgExplainClassification) {
                        this.sgClassificationRationale.append("[result: total positive; total negative]");
                    }
                } else {
                    this.igPositiveSentiment = iPosMax;
                    this.igNegativeSentiment = iNegMax;
                    if (this.options.bgExplainClassification) {
                        this.sgClassificationRationale.append("[result: max + and - of any sentence]");
                    }
                }
            }
            ...
            if (this.options.bgScaleMode) {
                ...
                if (this.options.bgExplainClassification) {
                    this.sgClassificationRationale.append("[scale result = sum of pos and neg scores]");
                }
                ...
                if (var10000 == 2) {
                    if (this.igPositiveSentiment == 0 && this.igNegativeSentiment == 0) {
                        if (this.options.bgBinaryVersionOfTrinaryMode) {
                            ...
                            if (this.options.bgExplainClassification) {
                                this.sgClassificationRationale.append("[binary result set to default value]");
                            }
                        } else {
                            this.igTrinarySentiment = 0;
                            if (this.options.bgExplainClassification) {
                                this.sgClassificationRationale.append("[trinary result 0 as pos=1, neg=-1]");
                            }
                        }
                    } else {
                        ...
                        if (this.options.bgBinaryVersionOfTrinaryMode) {
                            this.igTrinarySentiment = this.options.igDefaultBinaryClassification;
                            if (this.options.bgExplainClassification) {
                                this.sgClassificationRationale.append("[binary result = default value as pos = -neg * " +
                                        this.options.fgNegativeSentimentMultiplier + "]");
                            }
                        } else {
                            this.igTrinarySentiment = 0;
                    ...
                    if (this.igPositiveSentiment == 1 && this.igNegativeSentiment == -1) {
                        if (this.options.bgBinaryVersionOfTrinaryMode) {
                            this.igTrinarySentiment = this.binarySelectionTieBreaker();
                            if (this.options.bgExplainClassification) {
                                this.sgClassificationRationale.append("[binary result = default value as pos=1 neg=-1]");
                            }
                        } else {
                            this.igTrinarySentiment = 0;
                            if (this.options.bgExplainClassification) {
                    ...
                    if (this.options.bgBinaryVersionOfTrinaryMode && iPosTot == -iNegTot) {
                        this.igTrinarySentiment = this.binarySelectionTieBreaker();
                        if (this.options.bgExplainClassification) {
                            this.sgClassificationRationale.append("[binary result = default as posSentenceTotal>-negSentenceTotal]");
                        }
                    } else {
                        if (this.options.bgExplainClassification) {
                            this.sgClassificationRationale.append("[overall result = largest of posSentenceTotal, negSentenceTotal]");
                        }
