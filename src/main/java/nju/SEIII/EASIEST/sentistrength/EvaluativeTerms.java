...
public class EvaluativeTerms {
    ...
    public boolean initialise(String sSourceFile, ClassificationOptions options, IdiomList idiomList, SentimentWords sentimentWords) {
        ...
        if (igObjectEvaluationCount > 0) return true;
        ...
        try {
            ...
            else rReader = new BufferedReader(new FileReader(sSourceFile));
            ...
            rReader.close();
            if (igObjectEvaluationCount > 0) options.bgUseObjectEvaluationTable = true;
            if (bSentimentWordsAdded) sentimentWords.sortSentimentList();
            if (bIdiomsAdded) idiomList.convertIdiomStringsToWordLists();
