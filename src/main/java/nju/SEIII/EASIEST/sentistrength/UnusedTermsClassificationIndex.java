...
public class UnusedTermsClassificationIndex {
  ...
  public void addTermToNewTermIndex(String sTerm) {
    ...
    int iTermID =
        Trie.i_GetTriePositionForString(sTerm, sgTermList, igTermListLessPtr, igTermListMorePtr, 1,
            igTermListCount, bDontAddMoreElements);
    if (iTermID > 0) {
   ...
   */
  public void addNewIndexToMainIndexWithPosNegValues(int iCorrectPosClass, int iEstPosClass,
                                                     int iCorrectNegClass, int iEstNegClass) {
      ...
      for (int iTerm = 1; iTerm <= iTermsAddedIDTempCount; iTerm++) {
        ...
        if (igTermListFreqTemp[iTermID] != 0) {
          try {
            ...
            System.out.println(
                "[UnusedTermsClassificationIndex] Error trying to add Pos + Neg to index. " +
                    e.getMessage());
          }
        }
      }
   ...
   */
  public void addNewIndexToMainIndexWithTrinaryValues(int iCorrectTrinaryClass,
   ...
   */
  public void initialise(boolean bInitialiseScale, boolean bInitialisePosNeg,
                         boolean bInitialiseBinary, boolean bInitialiseTrinary) {
    igTermListCount = 0;
    igTermListMax = 50000;
    iTermsAddedIDTempCount = 0;
  ...
  public void printIndexWithPosNegValues(String sOutputFile, int iMinFreq) {
    try {
      ...
      wWriter.write("Term\tTermFreq >= " + iMinFreq + "\t" + "PosClassDiff (correct-estimate)\t" +
          "NegClassDiff\t" + "PosClassAvDiff\t" + "NegClassAvDiff\t");
      for (int i = 1; i <= 5; i++) {
      ...
      if (igTermListCount > 0) {
        for (int iTerm = 1; iTerm <= igTermListCount; iTerm++) {
          if (igTermListFreq[iTerm] >= iMinFreq) {
            wWriter.write(sgTermList[iTerm] + "\t" + igTermListFreq[iTerm] + "\t" +
                igTermListPosClassDiff[iTerm] + "\t" + igTermListNegClassDiff[iTerm] + "\t" +
                (float) igTermListPosClassDiff[iTerm] / (float) igTermListFreq[iTerm] + "\t" +
